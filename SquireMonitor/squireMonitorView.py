...
class SquireMonitorView(tk.Frame):
	def __init__(self, master=None):
		...
		self.systemDisplayNames = {TestSystemNames.Squire1: "SQ1", TestSystemNames.Squire2: "SQ2",
								...
								TestSystemNames.Squire9: "SQ9"}

		self.testDisplayNames = {TestNames.Drive: "DRIVE", TestNames.Loop: "LOOP", TestNames.IVVR: "IVVR"}						
		self.labelColors = {"Failure": "red", "Success": "green", "Not found": "grey"}
		self.resultImage = {"Success":checkimage, "Not found":questionimage, "Failure":redcrossimage}
	...
	def _update_label(self, r, c, testSystemName, result):
		...
		labelText = self.systemDisplayNames[testSystemName] + " " + self.testDisplayNames[result.testName] + " " + result.get_latest_result_date()
		...
		if label == 0:
			...
			label = tk.Label(self, textvariable=content, anchor=tk.W, padx=20, compound=tk.LEFT, relief=tk.RAISED, bg=self.labelColors[latestResult], image=self.resultImage[latestResult], font=('Helvetica', 17, 'bold'))
